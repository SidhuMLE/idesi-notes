    ...
    <div className="bg-temple-ivory min-h-screen flex flex-col">
      <header className="w-full top-0 sticky bg-temple-ivory z-40 border-b border-pandya-gold/10">
        <div className="flex justify-between items-center px-6 py-4">
          <button
            onClick={() => navigate(-1)}
            className="text-granite hover:text-primary active:scale-95 p-2 -ml-2 rounded-full"
          >
          ...
          <div className="grid grid-cols-4 gap-4 bg-surface-container-low p-4 rounded-xl border border-stone/10 shadow-[0_4px_20px_rgba(34,30,26,0.03)]">
            {ICONS.map((ic) => (
              <button
                ...
                className={`aspect-square flex flex-col items-center justify-center rounded-lg transition-all active:scale-95 ${
                  icon === ic
          ...
          <div className="flex gap-3 flex-wrap">
            {COLORS.map((c) => (
              <button
                ...
                className={`w-10 h-10 rounded-full transition-all active:scale-95 flex items-center justify-center ${
